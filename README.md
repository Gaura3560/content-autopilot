...
```
┌─────────────────────────────────────────────────────────┐
│  Content Autopilot Dashboard              v9.0 | 2026  │
├─────────────────────────────────────────────────────────┤
│                                                         │
│  ┌──────┐ ┌──────┐ ┌──────┐ ┌──────┐ ┌──────┐         │
│  │  5   │ │ 90.0 │ │  ▲   │ │ Num  │ │  2d  │         │
│  │Runs  │ │Qual. │ │Trend │ │Logic │ │Streak│         │
│  └──────┘ └──────┘ └──────┘ └──────┘ └──────┘         │
│                                                         │
│  Today's Content     Quality Scores                     │
│  ┌────┐┌────┐┌────┐ Hook     ████████████ 10/10        │
│  │Note││ X  ││ IG │ Read.    ████████░░░░  8/10        │
│  │C 70││B 86││B 86│ Struct.  █████████░░░  9/10        │
│  └────┘└────┘└────┘ CTA      ████████████ 10/10        │
│                                                         │
│  Funnel Balance                                         │
│  Actual: TOFU 40% ████  MOFU 40% ████  BOFU 20% ██    │
│  Target: TOFU 50% █████ MOFU 30% ███   BOFU 20% ██    │
└─────────────────────────────────────────────────────────┘
```
...
| **X (Twitter)** | スレッド（5-7ツイート） | 140文字制限、番号付きフォーマット、最終ツイートにnote誘導 |
...
noteの「クリエイターエコノミー」と完全に整合するファネル設計。TOFU/MOFUでフォロワーを獲得し、BOFU（有料記事）で収益化。クリエイターが創作に集中できる環境を、コンテンツ制作の自動化で実現する。
...
### note記事を公開する
1. `note_YYYY-MM-DD.md` を開く
2. [note.com](https://note.com) → 「投稿」→ テキストエディタにマークダウンを貼り付け
3. 有料/無料はファネルステージに基づき自動判定済み（BOFU = 有料、それ以外 = 無料）
4. 「この記事でわかること」リストが冒頭に配置済み

### Xスレッドを投稿する
1. `x_YYYY-MM-DD.md` を開く
2. `1/N` から順に各ツイートをコピー＆投稿（`---` で区切り済み）
3. 最終ツイートにnote記事への誘導CTA配置済み

### Instagramに投稿する
1. `instagram_YYYY-MM-DD.md` を開く
...
| `/autopilot` | 同上（エイリアス） |
| `/setup-profile` | テーマ・文体・ターゲットをカスタマイズ |
