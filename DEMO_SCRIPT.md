...
**[0:00]**
Content Autopilot。1コマンドでnote・X・Instagramを自律生成します。

**[0:10]**
AIに書かせてもAI臭い。毎回プロンプトを考える。品質がブレる。これを解決します。

**[0:20]**
`/daily-autopilot`。これだけ。ここから人間は何もしません。

**[0:30]**
プロフィール自動生成。ファネル分析でTOFU不足を検出、TOFUコンテンツに自動決定。WebSearchでトピックを自動選択。人間はトピックを選んでいません。

**[0:50]**
note・X・Instagram、3プラットフォーム同時生成。

**[1:00]**
最大の差別化。6軸の品質採点が自動で走ります。スコアが閾値を下回ると、自動で改善。68点から82点へ。人間は何もしていません。

**[1:20]**
ダッシュボードが自動表示。品質スコア、ファネルバランス、実行履歴を一覧で確認できます。

**[1:35]**
Claude直接なら56点。Content Autopilotなら94点。同じClaudeでも38点の差。

**[1:50]**
外部連携。Notion、Gmail、Gemini画像生成、Google Analytics、Calendar。全て自動。

**[2:10]**
12本のPythonスクリプト。6軸品質採点、AI臭10パターン検出、HTMLダッシュボード。23テスト全通過。

**[2:30]**
テーマ「一度命じたら、あとは任せろ」。7つの判断を全てシステムが自律的に行います。

**[2:50]**
Content Autopilot。ありがとうございました。
