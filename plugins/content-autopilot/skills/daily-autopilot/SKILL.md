...
STATE: LEARN_STYLE (Step 0.5 — 文体学習)
  ...
  → 未学習の場合:
    ...
    → 3の場合: Readツールでファイルを読み込み
    → 4の場合: スキップしてデフォルト文体を使用
...
STATE: GENERATE (Step 4/8)
  ...
  → profile.jsonのstyle.analysisがある場合、その文体を忠実に再現する
...
- スレッド番号付き（1/5, 2/5...）
- **X記事（Articles）も生成する:**
  - X Premium/Premium+ユーザー向けの長文記事フォーマット
  - アルゴリズムで優遇（プラットフォーム滞在時間を伸ばすため）
  - 通常の外部リンクよりCTRが高い専用レイアウトで表示される
  - スレッドに加えて x_article_{date}.md も生成する
  - 内容はnote記事を短縮した1000-2000文字版
- **X最新アルゴリズム対応（2026年版）:**
